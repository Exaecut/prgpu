...
impl AdobePluginGlobal for Plugin {
	...
	fn handle_command(&mut self, command: Command, in_data: InData, mut out_data: OutData, params: &mut Parameters<Params>) -> Result<(), Error> {
		match command {
			...
			ae::Command::About => {
				let msg = format!("Exaecut - Chromatic Aberration\r\nVersion: {}", env!("CARGO_PKG_VERSION"));
			...
			ae::Command::UserChangedParam { param_index } => {
				...
				if params.type_at(param_index) == Params::Help {
					let _ = webbrowser::open("https://exaecut.io/docs/40");
				}

				if params.type_at(param_index) == Params::Feedback {
					let _ = webbrowser::open("https://exaecut.io/feedback/40");
				}
			...
			ae::Command::SmartPreRender { mut extra } => {
				...
				let clip = params.get(Params::Clip)?.as_checkbox()?.value();
				let spread = if clip {
					0.0
				} else {
					params.get(Params::Spread)?.as_float_slider()?.value() as f32
				};
					...
					.checkout_layer(0, 0, &req, in_data.current_time(), in_data.time_step(), in_data.time_scale())
				{
					...
					res_rect.set_origin(ae::Point {
						h: res_rect.origin().h - (res_rect.width() as f32 * ((spread / 100.0) * 1.0)) as i32,
						v: res_rect.origin().v - (res_rect.height() as f32 * ((spread / 100.0) * 1.0)) as i32,
					});

					res_rect.set_width(res_rect.width() + (res_rect.width() as f32 * ((spread / 100.0) * 1.0)) as i32);
					res_rect.set_height(res_rect.height() + (res_rect.height() as f32 * ((spread / 100.0) * 1.0)) as i32);
