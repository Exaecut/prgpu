...
pub struct KernelParams {
	debug: u32,         // 0 or 1
	preview_layer: u32, // 1 Final Output, 2 Threshold Mask, 3 Echoes only
	clip_bounds: u32,   // 0 no clip, 1 clip
	blend_mode: u32,    // 1 Add, 2 Screen, 3 Overlay, 4 Color Dodge

	// Echo and transform
	echo: u32,                     // number of echoes
	transform_position: Vec2, // pixels per echo (dx, dy)
	transform_rotation: f32,       // degrees per echo
	transform_scale: f32,          // percent per echo - 100 means no scale

	// Look controls
	threshold: f32,            // 0..100
	threshold_smoothness: f32, // 0..1
	exposure: f32,             // multiplier
	decay: f32,                // 0..1 multiplicative per echo
	tint_color: Vec4,     // 0..1 RGBA

	// Blur
	bluriness: f32, // 0..100 - additive per echo in px scaled internally
}

impl KernelParams {
	pub fn from_params(
		params: &mut Parameters<Params>,
		_downsamples: (f32, f32),
		...
		Ok(Self {
			...
			echo: params.get(Params::Echo)?.as_float_slider()?.value() as u32,
			transform_position: Vec2 {
				x: params.get(Params::TransformPosition)?.as_point()?.value().0,
				y: params.get(Params::TransformPosition)?.as_point()?.value().1,
			},
			transform_rotation: params.get(Params::TransformRotation)?.as_angle()?.value(),
			transform_scale: params.get(Params::TransformScale)?.as_float_slider()?.value() as f32,
