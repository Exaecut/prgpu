		...
		match command {
			ae::Command::GlobalSetup => {
				...
				if let Ok(suite) = aegp::suites::Utility::new() {
					self.plugin_id = suite.register_with_aegp(None, "EXAE Godrays")?;
				}
			...
			ae::Command::About => {
				let msg = format!("Exaecut - Godrays\r\nVersion: {}", env!("CARGO_PKG_VERSION"));
			...
			ae::Command::UserChangedParam { param_index } => {
				...
				if params.type_at(param_index) == Params::Feedback {
					let _ = webbrowser::open("https://exaecut.io/feedback/45");
				}

				if params.type_at(param_index) == Params::NoLicense {
					let _ = webbrowser::open("https://exaecut.io/no-license?id=45");
				}
			}
			ae::Command::FrameSetup { in_layer, .. } => {
				...
				let length = params.get(Params::Length)?.as_float_slider()?.value() as f32;
				let length_multiplier = params.get(Params::LengthMultiplier)?.as_float_slider()?.value() as f32;
				let clip_bounds = params.get(Params::ClipBounds)?.as_checkbox()?.value();
				let extension = if !clip_bounds {
					((length * length_multiplier) * 2.0).round() as i32
				} else {
					0
				};
				...
				let rect_center = (
					out_data.origin().h as f32,
					out_data.origin().v as f32,
				);
			...
			ae::Command::SmartPreRender { mut extra } => {
					...
					.checkout_layer(0, 0, &req, in_data.current_time(), in_data.time_step(), in_data.time_scale())
				{
					...
					let rect_center = (
						res_rect.origin().h as f32,
						res_rect.origin().v as f32,
					);
