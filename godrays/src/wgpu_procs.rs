...
impl KernelParams {
	pub fn from_params(params: &mut Parameters<Params>, time: f32, in_data: after_effects::InData, rect_center: (f32, f32), _downsamples: (f32, f32)) -> Result<Self, after_effects::Error> {
		...
		Ok(Self {
			...
			key_color: Vec4 {
				x: (key_color.red as f32) / 255.0,
				y: (key_color.green as f32) / 255.0,
				z: (key_color.blue as f32) / 255.0,
				w: (key_color.alpha as f32) / 255.0,
			},
			key_color_sensitivity: params.get(Params::KeyColorSensitivity)?.as_float_slider()?.value() as f32,
			exposure: params.get(Params::Exposure)?.as_float_slider()?.value() as f32,
			decay: params.get(Params::Decay)?.as_float_slider()?.value() as f32,
			length: params.get(Params::Length)?.as_float_slider()?.value() as f32,
			length_multiplier: params.get(Params::LengthMultiplier)?.as_float_slider()?.value() as f32,
			center: Vec2 {
				x: params.get(Params::Center)?.as_point()?.value().0 - rect_center.0,
				y: params.get(Params::Center)?.as_point()?.value().1 - rect_center.1,
			},
			samples: params.get(Params::Samples)?.as_float_slider()?.value() as f32,
			tint_color: Vec4 {
			...
			blend_mode: params.get(Params::BlendMode)?.as_popup()?.value() as u32,
			bluriness: params.get(Params::Bluriness)?.as_float_slider()?.value() as f32,
			clip_bounds: params.get(Params::ClipBounds)?.as_checkbox()?.value() as u32,
		})
	}
}
...
impl<T: Sized> WgpuProcessing<T> {
	...
	fn run_main_pass(&self, encoder: &mut CommandEncoder, state: &BufferState, params: &KernelParams, out_size: (usize, usize, usize)) {
		self.queue.write_buffer(&state.main_params, 0, params.as_std140().as_bytes());
	...
	pub fn run_compute(&self, params: &KernelParams, in_size: (usize, usize, usize), out_size: (usize, usize, usize), in_buffer: &[u8], out_buffer: &mut [u8]) -> bool {
		let key = BufferKey { in_size, out_size };
