...
fn smooth_cubic_log(x: f32) -> f32 {
    if (x <= 1.0) {
        return x * x * x;
    } else {
        return 3.0 * log(x) + 1.0;
    }
}

fn blendOver(a: vec4<f32>, b: vec4<f32>) -> vec4<f32> {
    let newAlpha = mix(b.r, 1.0, a.r);
    let newColor = mix(b.r * b.gba, a.gba, a.r);
    let divideFactor = select(1.0, 1.0 / newAlpha, newAlpha > 0.001);
    return vec4<f32>(newAlpha, divideFactor * newColor);
}
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    uv -= ((params.real_radius * params.downsampling_factor) * texel_size).xy;
    ...
    if (uv.x < 0.0 || uv.x > 1.0 || uv.y < 0.0 || uv.y > 1.0) {
        original_color.r = 0.0;
    }

    var bloom_color = chromatic_aberration_sample(out_uv, output_size_f32) * vec4<f32>(1.0, params.tint_color_r, params.tint_color_g, params.tint_color_b);
    var bloom_alpha = rgb_to_lightness(bloom_color.gba) * smooth_cubic_log(params.bloom_strength * 0.7);
    ...
    if params.debug == 1u {
        ...
        let bloom_rgb = bloom_color.gba;

        switch (params.preview_layer - 1u) {
            case 0u: {
                var bloom_tonemapped = vec4<f32>(bloom_alpha, bloom_rgb);

                //premult
                bloom_tonemapped.g *= bloom_tonemapped.r;
                bloom_tonemapped.b *= bloom_tonemapped.r;
                bloom_tonemapped.a *= bloom_tonemapped.r;

                // unmult
                let max_comp = max(bloom_tonemapped.g, max(bloom_tonemapped.b, bloom_tonemapped.a));
                bloom_tonemapped.g /= max_comp;
                bloom_tonemapped.b /= max_comp;
                bloom_tonemapped.a /= max_comp;

                bloom_tonemapped = saturate(bloom_tonemapped);
                
                if (params.flicker == 1u) {
                    let flicker_factor = flicker(params.time, params.flicker_frequency, params.flicker_randomness);
                    bloom_tonemapped.r *= flicker_factor + params.flicker_bias;
                }
                
                out_color = vec4<f32>((bloom_tonemapped.r) + original_color.r, original_color.gba + (bloom_tonemapped.gba * bloom_alpha));
                break;
            }
            case 1u: {
                // Bloom only
                ...
                //premult
                bloom_tonemapped.g *= bloom_tonemapped.a;
                bloom_tonemapped.b *= bloom_tonemapped.a;
                bloom_tonemapped.a *= bloom_tonemapped.a;
            ...
            case 2u : {
                var bloom_tonemapped = bloom_color;
