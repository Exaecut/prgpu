...
pub struct KernelParams {
    ...
    pub threshold1: f32,           // First luminance threshold (default: 0.0)
    pub threshold2: f32,           // Second luminance threshold (default: 0.5)
    pub increment_base: f32,       // Base increment divisor (default: 30.0)
    pub increment_random_range: f32 // Random increment range (default: 6.0)
}
...
impl KernelParams {
	pub fn from_params(params: &mut Parameters<Params>, time: f32, in_data: after_effects::InData, downsamples: (f32, f32)) -> Result<Self, after_effects::Error> {
		...
		let threshold1 = params.get(Params::DarkLuminanceThreshold)?.as_float_slider()?.value() as f32;
		let threshold2 = params.get(Params::LightLuminanceThreshold)?.as_float_slider()?.value() as f32;
		let increment_base = params.get(Params::IncrementDivisor)?.as_float_slider()?.value() as f32;
		let increment_random_range = params.get(Params::RandomIncrementRange)?.as_float_slider()?.value() as f32;

		Ok(Self {
			time,
			debug: debug_flag,
			threshold1: threshold1 / 100.0,
			threshold2: threshold2 / 100.0,
			increment_base,
			increment_random_range,
