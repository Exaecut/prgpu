	...
	fn default() -> Self {
		Self {
			wgpu: if cfg!(shader_hotreload) && cfg!(debug_assertions) {
				WgpuProcessing::new(
					utils::prepare_shader("turbulent_depth"),
					utils::prepare_shader("energy_flux"),
					utils::prepare_shader("sun"),
					utils::prepare_shader("velvet"),
				)
			} else {
				WgpuProcessing::new(
					ProcShaderSource::Wgsl(include_str!("../shaders/turbulent_depth.wgsl").to_string()),
					ProcShaderSource::Wgsl(include_str!("../shaders/energy_flux.wgsl").to_string()),
					ProcShaderSource::Wgsl(include_str!("../shaders/sun.wgsl").to_string()),
					ProcShaderSource::Wgsl(include_str!("../shaders/velvet.wgsl").to_string()),
				)
			},
...
struct FrameData {
	...
	energy_flux_params: EnergyFluxParams,
	the_sun_params: TheSunParams,
	velvet_params: VelvetParams,
}
		...
		match command {
			ae::Command::GlobalSetup => {
				...
				if let Ok(suite) = aegp::suites::Utility::new() {
					self.plugin_id = suite.register_with_aegp(None, "EXAE Plasma Waves")?;
				}
			...
			ae::Command::About => {
				let msg = format!("Exaecut - Plasma Waves\r\nVersion: {}", env!("CARGO_PKG_VERSION"));
				out_data.set_return_msg(msg.as_str());
			}
			ae::Command::UpdateParamsUi => {
				let plasma_mode = params.get(Params::PlasmaMode)?.as_popup()?.value() as u32;
				let plasma_modes = params.get(Params::PlasmaMode)?.as_popup()?.options().len() as u32;

				let mut params_copy = params.clone();

				for i in 1..plasma_modes + 1 {
					let mut plasma_mode_params = params_copy.get_mut(Params::Start(PlasmaModes::from_u32(i)))?;
					plasma_mode_params.set_ui_flag(ParamUIFlags::INVISIBLE, plasma_mode != i);
					plasma_mode_params.update_param_ui()?;
				}
				...
				if in_data.is_after_effects() {
					let effect = in_data.effect();
					{
						...
						for i in 1..plasma_modes + 1 {
							let param_stream = aegp_plugin.new_stream_by_index(plugin_id, params.index(Params::Start(PlasmaModes::from_u32(i))).unwrap() as _)?;
							param_stream.set_dynamic_stream_flag(aegp::DynamicStreamFlags::Hidden, false, plasma_mode != i)?;
						}
					}
				}
			...
			ae::Command::UserChangedParam { param_index } => {
				...
				if params.type_at(param_index) == Params::Feedback {
					let _ = webbrowser::open("https://exaecut.io/feedback/42");
				}
			...
			ae::Command::FrameSetup { in_layer, .. } => {
				...
				out_data.set_frame_data::<FrameData>(FrameData {
					...
					energy_flux_params: EnergyFluxParams::from_params(params, time as f32, in_data)?,
					the_sun_params: TheSunParams::from_params(params, time as f32, in_data)?,
					velvet_params: VelvetParams::from_params(params, time as f32, in_data)?,
				});
			}
			...
			ae::Command::Render { in_layer, mut out_layer } => {
				...
				match PlasmaModes::from_u32(plasma_mode) {
					PlasmaModes::TurbulentDepth => {
						self.wgpu
							.run_compute::<KernelParams>(&frame_data.main_params, in_size, out_size, in_layer.buffer(), out_layer.buffer_mut())
					}
					PlasmaModes::EnergyFlux => {
						self.wgpu
							.run_compute::<EnergyFluxParams>(&frame_data.energy_flux_params, in_size, out_size, in_layer.buffer(), out_layer.buffer_mut())
					}
					PlasmaModes::TheSun => {
						self.wgpu
							.run_compute::<TheSunParams>(&frame_data.the_sun_params, in_size, out_size, in_layer.buffer(), out_layer.buffer_mut())
					}
					PlasmaModes::SweetVelvet => {
						self.wgpu
							.run_compute::<VelvetParams>(&frame_data.velvet_params, in_size, out_size, in_layer.buffer(), out_layer.buffer_mut())
					}
				};
			}
