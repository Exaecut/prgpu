use after_effects::{log, Parameters, Rect};
...
impl KernelParams {
	pub fn from_params(params: &mut Parameters<Params>, time: f32, _downsampling: (f32, f32), in_data: after_effects::InData, rect: Option<Rect>, xframe: Option<f32>) -> Result<Self, after_effects::Error> {
		...
		let inner_spread = params.get(Params::InnerSpread)?.as_float_slider()?.value() as f32;
		let outer_spread = params.get(Params::OuterSpread)?.as_float_slider()?.value() as f32;
		let spread_fade = params.get(Params::SpreadFade)?.as_float_slider()?.value() as f32;
		let blur_type = params.get(Params::Type)?.as_popup()?.value() as u32;
		let samples = params.get(Params::Samples)?.as_float_slider()?.value() as u32;
		let mut origin = params.get(Params::Origin)?.as_point()?.value();

		if let Some(rect) = rect {
			origin.0 -= rect.origin().h as f32;
			origin.1 -= rect.origin().v as f32;
		}

		let red_offset = params.get(Params::RedOffset)?.as_angle()?.value() / 360.0;
		let green_offset = params.get(Params::GreenOffset)?.as_angle()?.value() / 360.0;
		let blue_offset = params.get(Params::BlueOffset)?.as_angle()?.value() / 360.0;

		let blur_alpha = params.get(Params::BlurAlpha)?.as_checkbox()?.value() as u32;
		let uniform_aspect_ratio = params.get(Params::UniformAspectRatio)?.as_checkbox()?.value() as u32;

		Ok(Self {
			time,
			angle: angle / 360.0,
			xframe: xframe.unwrap_or(0.0),
			inner_spread,
			outer_spread,
			spread_fade,
			blur_type,
