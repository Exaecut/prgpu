	...
	fn default() -> Self {
		Self {
			wgpu: if cfg!(shader_hotreload) && cfg!(debug_assertions) {
				let signal_pass = format!("{}/shaders/signal_noise.wgsl", std::env::current_dir().unwrap().display());
				let compress_shader_path = format!("{}/shaders/compress.wgsl", std::env::current_dir().unwrap().display());
				let main_shader_path = format!("{}/shaders/main.wgsl", std::env::current_dir().unwrap().display());
				log::info!("Loading shaders on the fly. \n{}", main_shader_path);

				let signal_pass_path = std::fs::read_to_string(signal_pass.clone()).unwrap();
				let signal_pass = ProcShaderSource::Wgsl(&signal_pass_path);

				let compress_pass_path = std::fs::read_to_string(compress_shader_path.clone()).unwrap();
				let compress_pass = ProcShaderSource::Wgsl(&compress_pass_path);

				let main_pass_path = std::fs::read_to_string(main_shader_path.clone()).unwrap();
				let main_pass = ProcShaderSource::Wgsl(&main_pass_path);

				WgpuProcessing::new(main_pass, compress_pass, signal_pass)
			} else {
				WgpuProcessing::new(
					ProcShaderSource::Wgsl(include_str!("../shaders/main.wgsl")),
					ProcShaderSource::Wgsl(include_str!("../shaders/compress.wgsl")),
					ProcShaderSource::Wgsl(include_str!("../shaders/signal_noise.wgsl")),
				)
			},
		}
	}
...
#[repr(C)]
struct FrameData {
	signal_params: SignalNoiseParams,
...
impl Plugin {
	fn reload_shaders(&mut self) {
		self.wgpu = if cfg!(shader_hotreload) && cfg!(debug_assertions) {
			let signal_shader_path = format!("{}/shaders/signal_noise.wgsl", std::env::current_dir().unwrap().display());
			let compress_shader_path = format!("{}/shaders/compress.wgsl", std::env::current_dir().unwrap().display());
			let main_shader_path = format!("{}/shaders/main.wgsl", std::env::current_dir().unwrap().display());
			log::info!("Loading shaders on the fly. \n{}", main_shader_path);

			let signal_pass_path = std::fs::read_to_string(signal_shader_path.clone()).unwrap();
			let signal_pass = ProcShaderSource::Wgsl(&signal_pass_path);
...
impl AdobePluginGlobal for Plugin {
	...
	fn handle_command(&mut self, command: Command, in_data: InData, mut out_data: OutData, params: &mut Parameters<Params>) -> Result<(), Error> {
		match command {
			...
			ae::Command::About => {
				let msg = format!("Exaecut - Retro VHS\r\nVersion: {}", env!("CARGO_PKG_VERSION"));
			...
			ae::Command::UserChangedParam { param_index } => {
				...
				if params.type_at(param_index) == Params::Help {
					let _ = webbrowser::open("https://exaecut.io/docs/38");
				}

				if params.type_at(param_index) == Params::Feedback {
					let _ = webbrowser::open("https://exaecut.io/feedback/38");
				}
			...
			ae::Command::FrameSetup { in_layer, .. } => {
				...
				out_data.set_frame_data::<FrameData>(FrameData {
					...
					compress_params: CompressionParams::from_params(params, time as f32, in_data)?,
					signal_params: SignalNoiseParams::from_params(params, time as f32, in_data)?,
				});
			}
			...
			ae::Command::SmartPreRender { mut extra } => {
					...
					.checkout_layer(0, 0, &req, in_data.current_time(), in_data.time_step(), in_data.time_scale())
				{
					...
					res_rect.set_width(res_rect.width());
					res_rect.set_height(res_rect.height());
