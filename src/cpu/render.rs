...
fn rayon_dispatch<P: Copy + Sync>(width: u32, height: u32, buffers: SafeBuffers, tp: FrameParams, user_params: &P, dispatch_fn: CpuDispatchFn) -> Result<(), ae::Error> {
	use rayon::prelude::*;
	...
	(0..height).into_par_iter().for_each(move |y| {
		log::info!("Scanning row y={}", y);
        
		for x in 0..width {
			unsafe {
				dispatch_fn(x, y, buf_ptr as *const *const c_void, tp_ptr as *const c_void, up_ptr as *const c_void);
			}
		}
	});
