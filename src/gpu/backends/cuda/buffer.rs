		...
		DeviceHandleInit::FromSuite((device_index, suite)) => {
			...
			let buf = if let Some(existing) = guard.get(&key) {
				*existing
				...
				let raw = suite.allocate_device_memory(device_index, length).unwrap_or_else(|e| {
					log::error!("[CUDA] GPUDevice suite allocation failed: {e:?}");
					std::ptr::null_mut()
				});
				if raw.is_null() {
					log::error!("[CUDA] buffer allocation failed for {}x{} bpp={} tag={}", width, height, bytes_per_pixel, tag);
				}
				let obj = BufferObj { raw };
				guard.insert(key, obj);
				obj
