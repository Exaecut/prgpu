...
//! layout, `color`/`packed` are arrays — not vectors — so no 16-byte vec
//! alignment). 80-byte header + 256-byte packed char block = 336 bytes, a
...
const _: () = assert!(core::mem::size_of::<TextOverlayParams>() == 336);
