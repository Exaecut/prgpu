...
macro_rules! declare_kernel {
	($name:ident, $user_params_ty:ty) => {
		...
		$crate::paste::paste! {
			unsafe extern "C" {
				/// Raw CPU dispatch entry point (FFI symbol generated by the
				/// `.vekl` → C++ → shared-lib / static-link pipeline).
				///
				/// Kept `pub` so external benchmark / testing crates (e.g.
				/// `prgpu::bench`) can obtain the function pointer directly
				/// without going through the After Effects wrapper.
			...
			/// Typed pointer to the CPU dispatch function, suitable for the
			/// `prgpu::bench` harness and any host-side direct driver.
			#[allow(non_upper_case_globals)]
